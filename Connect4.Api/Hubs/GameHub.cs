...
public class GameHub : ExtendedHub<IOnlineGameClient>, IOnlineGameServer
{
	...
	private string? GetGameIdHeader()
	{
		var id = HttpContext?.Request.Headers["GameId"].ToString();
	...
	{
		if ( !Guid.TryParse(
			GetGameIdHeader() ?? throw new PDException( BadRequest( "GameId header not specified." ) ),
			out var uuid ) )
		{
	...
	private string? GetPlayerHeader()
	{
		var player = HttpContext?.Request.Headers["Player"].ToString();
	...
	private Hue GetPlayer()
	{
		if ( !Enum.TryParse<Hue>(
			GetPlayerHeader() ?? throw new PDException( BadRequest( "Player header not specified." ) ),
			out var player ) )
		{
